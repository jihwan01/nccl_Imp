...
ncclResult_t ncclTopoTuneModel(struct ncclComm* comm, int minCompCap, int maxCompCap, struct ncclTopoGraph** graphs) {
  ...
  // TMA now follows LL128 execution style.
  comm->maxThreads[NCCL_ALGO_RING][NCCL_PROTO_TMA] = comm->maxThreads[NCCL_ALGO_RING][NCCL_PROTO_LL128];
  comm->maxThreads[NCCL_ALGO_TREE][NCCL_PROTO_TMA] = comm->maxThreads[NCCL_ALGO_TREE][NCCL_PROTO_LL128];
  ...
  for (int a=0; a<NCCL_NUM_ALGORITHMS; a++) {
    ...
    comm->threadThresholds[a][NCCL_PROTO_TMA] = NCCL_LL128_THREAD_THRESHOLD;
  }
